...
resource "aws_autoscaling_group" "frontend-asg" {
  ...
  # Launch Template
 launch_template {
    id      = data.aws_launch_template.frontend_lt.id
    version = "$Latest"  # Use latest version of LT
  }
  # Instance Refresh
   instance_refresh {
    strategy = "Rolling"
...
# Fetch existing Launch Template
data "aws_launch_template" "frontend_lt" {
  name = "frontend"  # Name of your existing LT in AWS
}
...
launch_template {
    id      = data.aws_launch_template.backend_lt.id
    version = "$Latest"  # Use latest version of LT
  }
...
data "aws_launch_template" "backend_lt" {
  name = "backend"  # Name of your existing LT in AWS
}